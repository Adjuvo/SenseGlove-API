...
namespace CsCoreClient
{
	class Program
	{
		...
		{
            ////-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            //// Do not upload this ya bugger!

            //{
            //	SGCore.Nova.NovaGlove testGlove;
            //	if (SGCore.Nova.NovaGlove.GetNovaGlove(out testGlove))
            //             {
            //		bool rightHand = testGlove.IsRight();    //Returns true if this is a right hand. If false, this is a left hand.
            ...
            //		SGCore.Nova.Nova_SensorData sData;
            //		if (testGlove.GetSensorData(out sData))
            //                 {
            //			string debug = "[";
            //			float offset = 2048f;
            //			float[] currentFlex = new float[sData.SensorValues.Length];
            //			for (int f=0; f<currentFlex.Length; f++)
            //                     {
            //				currentFlex[f] = sData.SensorValues[f].y;
            //				debug += currentFlex[f].ToString();
            //				if (f < currentFlex.Length - 1)
            //                         {
            //					debug += ", ";
            //                         }
            //                     }

            //			Console.WriteLine("Latest Flexes" + debug + "]");
            //			//Set Thresholds to those values + 250 or so


            //			//Only for the index & middle finger for now...
            //			testGlove.QueueThresholdCmd(Finger.Index, currentFlex[1] + offset);
            //			testGlove.QueueFFBLevel(Finger.Thumb, 1.0f);
            //			testGlove.SendHaptics();
            //			Console.WriteLine("Sent Thresholds for index");

            //			Console.ReadKey();

            //			testGlove.QueueClearThreshold(Finger.Index); //turn index back off, middle back on
            //			testGlove.QueueThresholdCmd(Finger.Middle, currentFlex[2] + offset);
            //			testGlove.QueueVibroLevel(Finger.Thumb, 1.0f);
            //			testGlove.SendHaptics();

            //			Console.WriteLine("Cleared index, added Middle");

            //			Console.ReadKey();
            //                     testGlove.StopHaptics();
            //			Console.WriteLine("Ceased Haptics");
            //		}

            //		////testGlove.QueueWristCmd(0.10f);
            //		//testGlove.QueueVibroLevel(Finger.Index, 1.0f);
            //		//testGlove.SendCommands();

            //		//Console.ReadKey();

            //		//testGlove.SendCommands(); //sends the same commands. Ignores traditional
            //		//Console.ReadKey();
     ...
     //           if (SGCore.SG.SenseGlove.GetSenseGlove(out testGlove))
     //           {
     //               bool rightHand = testGlove.IsRight();    //Returns true if this is a right hand. If false, this is a left hand.
     //               string deviceID = testGlove.GetDeviceID();  //The Device's Unique Identifier. Useful if you have multiple of them.
     //               string type = testGlove.GetDeviceType().ToString(); //The DeviceType (SenseGlove DK1, SenseGlove Nova, etc.)
     ...
     //               //	testGlove.QueueVibroLevel(Finger.Thumb, 0.3f);
     //               float[] ffb = new float[5];
     //               ffb[0] = 0.6f;
     //               testGlove.QueueFFBLevels(ffb);
     //               testGlove.SendHaptics();
     //               Console.WriteLine("Thumb");
     //               Console.ReadKey();


     //               //	testGlove.QueueVibroLevel(Finger.Thumb, 0.4f);
     //               ffb = new float[5];
     //               ffb[1] = 0.6f;
     //               testGlove.QueueFFBLevels(ffb);
     //               testGlove.SendHaptics();
     ...
     //               //	testGlove.QueueVibroLevel(Finger.Thumb, 0.5f);
     //               ffb = new float[5];
     //               ffb[2] = 0.6f;
     //               testGlove.QueueFFBLevels(ffb);
     //               testGlove.SendHaptics();
     ...
     //               //	testGlove.QueueVibroLevel(Finger.Thumb, 0.6f);
     //               ffb = new float[5];
     //               ffb[3] = 0.6f;
     //               testGlove.QueueFFBLevels(ffb);
     //               testGlove.SendHaptics();
     ...
     //               //	testGlove.QueueVibroLevel(Finger.Thumb, 0.7f);
     //               ffb = new float[5];
     //               ffb[4] = 0.6f;
     //               testGlove.QueueFFBLevels(ffb);
     //               testGlove.SendHaptics();
     //               Console.WriteLine("Pinky");
     //               Console.ReadKey();
     ...
     //               testGlove.QueueWristCommand(SGCore.Haptics.SG_ThumperCmd.Impact_Thump_100);
     //               testGlove.SendHaptics();
     //               Console.WriteLine("Thumpah");
     //               Console.ReadKey();
