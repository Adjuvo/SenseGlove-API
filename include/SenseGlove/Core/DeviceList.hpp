...
public:
    ...
    /// <summary> Clear the device list, and block any further attempt to add to it until ReInitialize is called. </summary>
    ...
    static std::vector<std::shared_ptr<T>> GetDevices()
    {
        ...
        for (std::size_t i = 0; i < currentDevices; ++i) {
            ...
            if (typeid(d) == tType) {
                res.emplace_back(std::dynamic_pointer_cast<T>(GetSGDevices()[i])); //emplace_back is valid here, as we don't know how many devices are of the same type.
            }
        }
        return res;
    }
