...
public:
    ...
    [[nodiscard]] std::string GetForHardwareVersion() const;
...
public:
    ...
    /// <returns></returns>
    [[nodiscard]] bool MatchesDevice(
        bool bRight, SGCore::EDeviceType type, const std::string& hardwareVersion) const;

    /// <summary> Returns true if this pose was made for the same device as another pose. </summary>
    /// <param name="otherPose"></param>
    /// <returns></returns>
    [[nodiscard]] bool MatchesDevice(const CVHandTrackingData& otherPose) const;
    ...
    /// <returns></returns>
    [[nodiscard]] std::string PrintKeyPoints(const std::string& delimiter = ", ") const;
};
